...
/**
 * Tickle every subscription listening to a region. Dead endpoints (404/410)
 * are pruned. Runs in ctx.waitUntil — the poster never waits on fan-out.
 */
export async function notifyRegion(db, env, regionId) {
  const { results: subs } = await db.prepare(
    `SELECT s.id, s.endpoint FROM subscriptions s
